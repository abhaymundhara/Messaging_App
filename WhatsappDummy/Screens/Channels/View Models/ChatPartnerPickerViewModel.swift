...
final class ChatPartnerPickerViewModel: ObservableObject {
        ...
        Task {
            if let channelId = await verifyIfDirectChannelExists(with: chatPartner.uid) {
                ...
                case .failure(let failure):
                    ...
                    print("failed to create Direct channel: \(failure.localizedDescription)")
                }
            }
        }
    }
    ...
    func createGroupChannel(_ groupName: String?, completion: @escaping (_ newChannel: ChannelItem) -> Void) {
        ...
        case .success(let channel):
            completion(channel)
        case .failure(let failure):
            print("failed to create group channel: \(failure.localizedDescription)")
        }
    }
    ...
    private func showError(_ errorMessage: String) {
     ...
     private func createChannel(_ channelName: String?) -> Result<ChannelItem, Error> {
        ...
        membersUids.forEach { userId in
            //keeping index of the channel that a specific user belongs to
            FirebaseConstants.UserChannelsRef.child(currentUid).child(channelId).setValue(true)
            // makes sure direct channel is unique
